...
class cluster_disc(nn.Module):
    def __init__(self, K):
        ...
        self.model = nn.Sequential(
                ...
                nn.Linear(4*K, 2*K), #512, 256),
                nn.LeakyReLU(0.2, inplace=True),
                nn.Linear(2*K, 1), #256, 1),
                nn.Sigmoid()
                      )
...
class style_disc(nn.Module):
    def __init__(self, d_style):
        ...
        self.model = nn.Sequential(
                    nn.Linear(d_style, 4*d_style), #512),
                    nn.LeakyReLU(0.2, inplace=True),
...
#        print("use_teacher_forcing = ", use_teacher_forcing)
        ...
        if use_teacher_forcing:
            ...
            for di in range(out_len):
                ...
                ae_loss += criterion(dec_out.squeeze(), x[di, :, :]) # x as target
                dec_in = x[di, :, :].unsqueeze(0)  # x as target =  Teacher forcing    
        else:            
...
#                dec_in = topi.squeeze().detach()  # detach from history as input
                dec_in = dec_out.detach() #When the output becomes input  (should be detached)
                ae_loss += criterion(dec_out.squeeze(), x[di, :, :]) #Autoencoding
                
        return cluster, style, ae_loss
